...
void H264Frame::BeginNewFrame () //清空之前所有积攒
{
  _encodedFrameLen = 0;
...
bool H264Frame::SetFromRTPFrame(RTPFrame & frame, unsigned int & flags) {
  ...
  if (curNALType >= H264_NAL_TYPE_NON_IDR_SLICE &&
      curNALType <= H264_NAL_TYPE_FILLER_DATA) //一个NALU 封装一个RTP 
  {
  ...
  else if (curNALType == 24)  //多个NALU 封装成一个RTP
  {
    // stap-A (single time aggregation packet )
    return DeencapsulateSTAP (frame, flags);
  } 
  else if (curNALType == 28) //一个NALU 封装成多个RTP
  {
    // Fragmentation Units
...
void H264Frame::AddDataToEncodedFrame (unsigned char *data, unsigned int dataLen, unsigned char header, bool addHeader) //去除乱七八糟
{
